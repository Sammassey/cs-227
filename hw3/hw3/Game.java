...
public class Game
{
	...
	/**
	 * Holds the given size for the grid of the generated game given in the constructor method.
	 */
	private int[][] grid;

	/**
	 * Holds the configurations for the given GameUtil object given in the constructor method.
	 */
	private GameUtil config;

	/**
	 * Holds the given instance of random given in the constructor method.
	 */
	private Random rand;

	/**
	 * Holds the total score for the game. The total score is all the numbers on the tiles added together.
	 */
	private int score;
	...
	/**
	 * Holds the value for the next tiles.
	 */
	private int nextTileValue;

	/**
	 * Remembers the previous move direction in the game.
	 */
	private Direction lastMove;
	
	/**
	 * Holds the last amount of moves made in the game.
	 * If no moves have been made yet, this will be a null value.
	 */
	private int lastMoveAmountMade;
	
	/**
	 * A new grid that can save the variable grid to it before anything moves in said grid.
	 * This will remember the previous grid even after it is updated.
	 */
	private int[][] grid2;
	...
	public Game(int givenSize, GameUtil givenConfig, Random givenRandom)
	{
		...
		grid = config.initializeNewGrid(givenSize, rand);
	}
	...
	public ArrayList<Move> shiftGrid(Direction dir)
	{
		...
		if(dir == dir.LEFT)
		{
			...
			{
				int j = list.size();
				temp = copyRowOrColumn(i, dir.LEFT);
				list.addAll(config.shiftArray(temp));
				updateRowOrColumn(temp, i, dir.LEFT);
				while(j < list.size())
				{
					list.get(j).setDirection(i, dir);
					j++;
				}
			}
		}
		if(dir == dir.RIGHT)
		{
			...
			{
				int j = list.size();
				temp = copyRowOrColumn(i, dir.RIGHT);
				list.addAll(config.shiftArray(temp));
				updateRowOrColumn(temp, i, dir.RIGHT);
				while(j < list.size())
				{
					list.get(j).setDirection(i, dir);
					j++;
		...
		if(dir == dir.UP)
		{
			...
			{
				int j = list.size();
				temp = copyRowOrColumn(i, dir.UP);
				list.addAll(config.shiftArray(temp));
				updateRowOrColumn(temp, i, dir.UP);
				while(j < list.size())
				{
					list.get(j).setDirection(i, dir);
					j++;
		...
		if(dir == dir.DOWN)
		{
			...
			{
				int j = list.size();
				temp = copyRowOrColumn(i, dir.DOWN);
				list.addAll(config.shiftArray(temp));
				updateRowOrColumn(temp, i, dir.DOWN);
				while(j < list.size())
				{
					list.get(j).setDirection(i, dir);
					j++;
	...
	{
		if(lastMoveAmountMade == 0)
		{
			return null;
		}
		else
		{
			TilePosition t = config.generateRandomTilePosition(grid, rand, lastMove);
			t.setValue(nextTileValue);
			grid[t.getRow()][t.getCol()] = nextTileValue;
			nextTileValue = config.generateRandomTileValue(rand);
			score += config.getScoreForValue(nextTileValue);
			lastMove = null;
			return t;
		}
	}
