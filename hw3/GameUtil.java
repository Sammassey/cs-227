...
{  
	...
	{
		TilePosition t = new TilePosition(0, 0, 0);
		...
		if(lastMove == lastMove.LEFT)
		{
			...
			{
				arr[j] = grid[j][arr.length - 1];
			}
			while(arr[i] != 0)
			{
				i = rand.nextInt(arr.length - 1);
			}
			t = new TilePosition(i, arr.length - 1, 0);
		}
		
		if(lastMove == lastMove.RIGHT)
		{
			for(int j = 0; j < arr.length; j++)
			{
				arr[j] = grid[j][0];
			...
			t = new TilePosition(i, 0, 0);
		}
		
		if(lastMove == lastMove.UP)
		{
			for(int j = 0; j < arr.length; j++)
			{
				arr[j] = grid[arr.length - 1][j];
			}
			while(arr[i] != 0)
			{
				i = rand.nextInt(arr.length - 1);
			}
			t = new TilePosition(arr.length - 1, i, 0);
		}
		
		if(lastMove == lastMove.DOWN)
		{
			for(int j = 0; j < arr.length; j++)
			{
				arr[j] = grid[0][j];
		...
		for(int i = 0; i < arr.length - 1; i++)
		{
			if(arr[i] == 0)
			{
				for(int j = i; j < arr.length - 1; j++)
				{
					...
					{
						arr[j] = arr[j + 1];
						list.add(new Move(j + 1, j, arr[j + 1]));
					}
					else 
					{
						arr[j] = 0;
			...
			if(mergeValues(arr[i], arr[i + 1]) / 3 > 0)
			{
				list.add(new Move(i + 1, i, arr[i], arr[i + 1], mergeValues(arr[i], arr[i + 1])));
				...
				for(int k = m; k < arr.length - 1; k++)
				{
					...
					if(arr[k + 1] > 0)
					{
						list.add(new Move(k + 1, k, arr[k + 1]));
					}
					if(k == arr.length - 2)
					{
						arr[k + 1] = 0;
